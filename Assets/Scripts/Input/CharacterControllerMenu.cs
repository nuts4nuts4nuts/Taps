...
public class CharacterControllerMenu : Menu
{
    ...
    public override void Work()
    {
        ...
        for(int i = 0; i < ControllerDataScript.MAX_CONTROLLERS; ++i)
        {
            if(i < numDevices)
            {
                startSpheres[i].GetComponent<Renderer>().material.color = Color.gray;
            }
            else
            {
                startSpheres[i].GetComponent<Renderer>().material.color = Color.black;
            }

            if (controllerData.controllers[i].device != null)
            {
                startSpheres[i].GetComponent<Renderer>().material.color = controllerData.controllers[i].color;
            }
        }
    }
}
