...
public class BallScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if(whoHolds == Players.Invalid)
            GetComponent<Rigidbody2D>().velocity = Vector2.ClampMagnitude(GetComponent<Rigidbody2D>().velocity, currentSpeed);
    ...
    void Update()
    {
        ...
        {
            float lifePercentage = (pList[i].remainingLifetime / pList[i].startLifetime);
        ...
        {
            float lifePercentage = (pListOrbit[i].remainingLifetime / pListOrbit[i].startLifetime);
            float curvedPercentage = colorCurve.Evaluate(lifePercentage);
            Color lerpedColor = Color.Lerp(trailParticleColor, orbitParticleColor, curvedPercentage);
    ...
    public void IgnorePhysics(int layer, bool yOrN = true, bool resetOld = true)
    {
        ...
        GetComponent<Renderer>().material.color = GameManager.instance.charPanels[layerToPlayer].color;
    ...
    private void CountdownBallTimer()
    {
        ...
        else
        {
            ...
            SetParticleColor(GetComponent<Renderer>().material.color);
